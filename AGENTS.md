...
- `README.md` は利用者が迷わず使うための文書として扱う。実装上の事実、内部仕様、例外処理、データ形式、生成ファイル名、主キー、詳細なオプションなどを、利用者の具体的な行動に直結しないまま列挙しないこと。
- `README.md` に情報を追加する前に、「初回利用者が今すぐ判断・操作するために必要か」を確認する。必要性が薄い詳細は `docs/` やコードコメントに分け、README には要点と必要な導線だけを書くこと。
...
- 【重要】Git管理されるファイルには、ユーザーのホームディレクトリパス・ユーザー名・メールアドレスなどの個人情報を**絶対に**記載しないこと。パスを示す必要がある場合は `~/` や相対パス、またはリポジトリルートからの相対表記を徹底すること。
