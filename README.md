...
- **検索ボタン（虫眼鏡アイコン）**: サムネイル一覧を表示して検索バーにフォーカス
  ...
  - `meta:public` と `meta:no-public` は sora-gallery export 用の制御タグとして、動画選択中のタグ候補に常時表示されます。片方を付けると競合するもう片方は保存時に外れます。
...
- player は動画 manifest があればそれを優先して読みます。manifest がない場合は初回表示時に自動生成します。自動生成では表示継続を優先し、`duplicateStrategy` が `manual` の場合は重複した動画 ID の古い方を選びます。自動生成に失敗した場合は同じ条件で一時スキャンして表示します。重複を手動確認したい場合は `npm run generate:manifest` を実行してください。
